    ...
    File(outputName).bufferedWriter().use {
        val str = buildString {
            text.split(" ").forEach { word ->
                val workedWord = buildString {
                    if (word.isNotEmpty()) {
                        append(word[0])
                        (1 until word.length).forEach { char ->
                            if (word[char] in vowels.keys && word[char - 1] in consonants) {
                                append(vowels[word[char]])
                            } else append(word[char])
                        }
